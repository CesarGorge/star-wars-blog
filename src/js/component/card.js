    ...
    <div className="card p-2" style={{ minWidth: "300px" }}>
      ...
      <div className="card-body">
        ...
        <div className="d-flex justify-content-between">
          {props.endpoint === "people" ? (
            <Link to={`people/${props.item.uid}`} className="btn btn-primary">
              Learn more!
            </Link>
          ) : (
            <Link to={`planets/${props.item.uid}`} className="btn btn-primary">
              Learn more!
            </Link>
          )}
          <button
            type="button"
            onClick={() => {
              let url = `/${props.item.url.replace(
          ...
          >
            {store &&
            store.favorites.find((favorite, index) => {
              return favorite.url === `/${props.endpoint}/${props.item.url}`;
            }) ? (
